...
ECDSA_SIG *Key::Sign(const unsigned char *digest, int digest_len, const BIGNUM *k)
{
  BN_CTX *ctx = NULL;
  BIGNUM *kinv = NULL, *rp = NULL,  *X = NULL, *order = NULL, *halforder = NULL;
  ...
  if (k == NULL) {
    ...
    sig = ECDSA_do_sign_ex(digest, digest_len, kinv, rp, ec);
  }
...
Key::SignSync(const Arguments& args)
{
  ...
  if (args.Length() != 1 && args.Length() != 2) {
    return VException("One or two arguments expected: hash, [k]");
  }
  if (!Buffer::HasInstance(args[0])) {
    return VException("Argument 'hash' must be of type Buffer");
  }
  ...
  if (args.Length() == 2) {
    if (!Buffer::HasInstance(args[1])) {
      return VException("Argument 'k' must be of type Buffer");
    }
    ...
    k = BN_bin2bn(&k_data[0], k_len, NULL);
  }

  // Create signature
  ECDSA_SIG *sig = key->Sign(hash_data, hash_len, k);

  unsigned char *raw_sig = (unsigned char *)malloc(64);

  BN_bn2bin(sig->r, &raw_sig[0]);
  BN_bn2bin(sig->s, &raw_sig[32]);

  Buffer *sig_buf = Buffer::New(64);
  memcpy(Buffer::Data(sig_buf), raw_sig, 64);
  ...
  return scope.Close(sig_buf->handle_);
}
